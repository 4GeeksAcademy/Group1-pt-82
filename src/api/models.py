...
class Booking(db.Model):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    google_calendar_id: Mapped[Optional[str]] = mapped_column(String(255), index=True, nullable=True)
    ...
    airbnb_guest_last_name: Mapped[Optional[str]] = mapped_column(String(120), nullable=True)
    airbnb_checkin: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    airbnb_checkout: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    reservation_url: Mapped[Optional[str]] = mapped_column(String(1024), nullable=True)
    airbnb_guestpic_url: Mapped[Optional[str]] = mapped_column(String(1024), nullable=True)
    needs_manual_details: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, default=datetime.utcnow, onupdate=datetime.utcnow)
    ...
    __table_args__ = (
        ...
        UniqueConstraint("listing_id", "google_calendar_id", name="uq_booking_listing_googleid"),
    )
